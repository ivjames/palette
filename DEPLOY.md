...
Target: **https://palette.lab980.com** — served from the lab980 droplet (conventions in
...
deployment. Everything is driven by the operate CLI at `bin/palette`.

> Why not `provision-site`? That script scaffolds proxy-shaped sites (an app on
> a local port). This site has no app, so `palette setup` writes its own
> static vhost instead — same DNS/doctl, security-headers and certbot shape.

## One-time bring-up (on the droplet, as root)

```bash
git clone https://github.com/ivjames/palette /var/www/palette
ln -sf /var/www/palette/bin/palette /usr/local/bin/palette
palette setup
```

`palette setup` is idempotent and does, in order:

1. **DNS** — `doctl` A record `palette.lab980.com -> droplet IP` (skipped if it already
   exists; `--no-dns` to skip, `--ip` to override autodetect).
2. **nginx** — static vhost from `deploy/nginx.conf.template` installed as
   `/etc/nginx/sites-available/palette.lab980.com`, symlinked into `sites-enabled/`,
...
3. **TLS** — waits for DNS to resolve, then
   `certbot --nginx -d palette.lab980.com --redirect -n`. If DNS is still propagating it
...
```bash
palette status              # HEAD commit, live probe, cert days
health-check --site palette # the droplet-wide auditor also covers it
```

## Overrides

- `PALETTE_FQDN` — serve under a different name (default `palette.lab980.com`)
- `PALETTE_BRANCH` — deploy a different branch (default `main`)
